...
def test_lanenet(image_path, weights_path):
    ...
    binary_image = cv2.adaptiveThreshold(image_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                         cv2.THRESH_BINARY, 25, 0)
    # cv2.imshow('image', image_edge)
    # cv2.waitKey()

    binary_image = cv2.resize(binary_image, (512, 256), interpolation=cv2.INTER_LINEAR)
    binary_image = binary_image / 127.5 - 1.0
    binary_image = np.expand_dims(binary_image, -1)

    LOG.info('Image load complete, cost time: {:.5f}s'.format(time.time() - t_start))

    input_tensor = tf.placeholder(dtype=tf.float32, shape=[1, 256, 512, 1], name='input_tensor')
    ...
    with sess.as_default():
        ...
        binary_seg_image, instance_seg_image = sess.run(
            ...
            feed_dict={input_tensor: [binary_image]}
        )
...
if __name__ == '__main__':
    ...
    # init args
    args = init_args()
    args.image_path = '/home/st/Tusimple测试/1.jpg'
    args.weights_path = '/media/st/9C20-06C6/Weights/CU_xy_bin/tusimple_train.ckpt'
